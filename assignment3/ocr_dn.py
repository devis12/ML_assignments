...
class OcrDeepModel(Model):
  def __init__(self):
    ...
    self.conv3 = OcrConvolutional(32, 64, 5)     # out shape : (batch, 4,  2, 128)
    self.pool3 = MaxPool2D([2,2])                 # out shape : (batch, 2,  1, 128)
    self.flatten = Flatten()                      # out shape : (batch, 2*1*128)
    self.fc1 = OcrFullyConnected(2*1*64, 256)    # out shape : (batch, 512)
    self.dropout = Dropout(0.5)                   # out shape : unchanged
    self.fc2 = OcrFullyConnected(256, 32)         # out shape : (batch, 32)
...
train_target = tf.one_hot(train_target, 32)

test_target = encode_chars_to_nums(test_target[:,0])
test_target = tf.one_hot(test_target, 32)
...
sel_train_ds = tf.data.Dataset.from_tensor_slices(
    (sel_train_data, sel_train_target)).shuffle(10000).batch(100)

sel_test_ds = tf.data.Dataset.from_tensor_slices((sel_test_data, sel_test_target)).batch(50)

for l_rate in possible_learning_rates:
  print(("\n\nMODEL SELECTION with Adamax (learning rate = {})").format(l_rate))
  network = OcrDeepModel()

  network_loss = tf.keras.losses.CategoricalCrossentropy()

  network_optimizer = tf.keras.optimizers.Adamax(learning_rate=l_rate)
...
train_ds = tf.data.Dataset.from_tensor_slices(
    (train_data, train_target)).shuffle(10000).batch(100)

test_ds = tf.data.Dataset.from_tensor_slices((test_data, test_target)).batch(50)
...
network_optimizer = tf.keras.optimizers.Adamax(learning_rate=selected_learning_rate)

print(("\n\nFINAL TRAINING with Adamax (learning rate = {})").format(selected_learning_rate))
EPOCHS = 32
