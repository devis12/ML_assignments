...
def test_step(images, labels, model, loss_fn, print_pred):
  predictions = model(images, training=False)
  if(print_pred):#predictions to be printed
    pred_to_be_printed = np.array(decode_nums_to_chars(tf.argmax(predictions, axis=1).numpy()))
    pred_to_be_printed = pred_to_be_printed.reshape(len(pred_to_be_printed),1)
    df = pd.DataFrame(pred_to_be_printed)
...
# if final_testing set to True, test_step will print the predictions and test_ds is assumed to be a batch containing the whole dataset
def test_loop(test_ds, model, loss_fn, final_testing): 
  # reset the metrics for the next epoch
  test_loss_metric.reset_states()
  test_accuracy_metric.reset_states()

  for test_images, test_labels in test_ds:
    test_step(test_images, test_labels, model, loss_fn, final_testing)
...
possible_optimizers = [tf.keras.optimizers.Adamax, tf.keras.optimizers.Adam, tf.keras.optimizers.Adadelta, tf.keras.optimizers.Adagrad, tf.keras.optimizers.Nadam, tf.keras.optimizers.RMSprop]
possible_optimizers_names = ["Adamax", "Adam", "Adadelta", "Adagrad", "Nadam", "RMSprop"]
...
for i in range(len(possible_optimizers)):
  ...
  for l_rate in possible_learning_rates:
    ...
    sel_test_accuracy = test_loop(sel_test_ds, network, network_loss, False)
...
test_ds = tf.data.Dataset.from_tensor_slices((test_data, test_target)).batch(test_data.shape[0])
...
"""
  Testing
"""
test_accuracy = test_loop(test_ds, network, network_loss, True)
